...
| `SQL_COCKPIT_NOTIFICATIONS_LISTEN_PREFIX` | Process environment | HTTP URL prefix | `http://127.0.0.1:8090/`; a missing CLI value falls back to this default | Existing runtime + supervisor paths | Wrong value breaks live notifications feed. | Keep `127.0.0.1` and verify `/health` on that port. |
