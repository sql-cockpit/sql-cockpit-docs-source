...
`Cockpit.ListBridgeInvocations` and `Cockpit.GetBridgeInvocation` are granted to `CockpitInvoker`. `Cockpit.CancelBridgeInvocation` is granted to `CockpitAdministrator` because it can terminate active data movement.
...
| `POST` | `/api/sql-bridge/invocations/{id}/cancel` | `sqlBridge.invocations.cancel` | Request cancellation through `Cockpit.CancelBridgeInvocation`. The Agent permits this exact procedure under the `sql-bridge` write policy while rejecting unlisted SQL writes. The optional JSON `reason` is stored with the request. Hosted tenants route the write through the paired local Agent. |
...
| `POST` | `/api/sql-bridge/locks/kill` | `sqlBridge.locks.kill` | Kill a confirmed SQL Server session that owns a SQL Cockpit `FetchRemote` lock through `Cockpit.KillBridgeLockSession`. Running requests additionally require `allowRunning=true`. Hosted tenants route the write through the paired local Agent. |

The invocation and lock-list endpoints read the bridge control database and SQL Server lock/session DMVs only. In hosted/cloud mode, the API sends the bounded SQL through the paired Agent using the `sql.database.execute` capability with `purpose = sql-bridge`; the cloud API must not connect directly to the customer SQL Server. Local direct SQL mode is available only when explicitly enabled in the runtime. The cancellation and kill endpoints are destructive. **Cancel sync** is preferred for a healthy queued or running invocation because it requests Agent executor termination by invocation id. The kill endpoint refuses the current SQL session and requires the target to hold a SQL Cockpit `FetchRemote` lock. The Locks dashboard labels idle sessions **Kill stale session** and active requests **Force kill active session**; both require confirmation, and only the active variant passes `allowRunning=true`. After either action, wait for destination rollback and lock release before retrying the same table.
...
| SQL Bridge invocation cancellation | `/sql-bridge`; `POST /api/sql-bridge/invocations/{id}/cancel`; permission catalog entry `sqlBridge.invocations.cancel` | Invocation id GUID plus optional cancellation reason; permission granted by default only through the built-in Local Admin role | Standard User remains read-only; no automatic cancellation | Active Bridge Syncs and expanded History rows call `Cockpit.CancelBridgeInvocation` through the paired Agent. | High: the Agent executor is terminated and destination rollback can retain locks after the visible invocation changes state. | Confirm the destination and phase, click **Cancel sync**, then monitor invocation and lock status until rollback completes before retrying. |
| SQL Bridge lock dashboard | `/sql-bridge/locks`; `GET /api/sql-bridge/locks`; `POST /api/sql-bridge/locks/kill` | Read requires `sqlBridge.locks.view`; kill requires `sqlBridge.locks.kill`; kill body accepts `sessionId`, optional `lockResource`, optional `reason`, optional `allowRunning` | Disabled kill button for users without kill permission; stale kill sends `allowRunning=false`, force kill sends `allowRunning=true` | Lets operators see blocked destination refresh locks and lets approved admins release stale or explicitly confirmed active SQL sessions without manually querying DMVs. Hosted mode uses the paired Agent. | High: SQL Server rolls back the target session and releases all locks it owns; large rollback can be lengthy. | Match invocation, destination, session id, host/login/program, and request state. Prefer cancellation for healthy invocations. Force-kill active SQL only after confirming impact, then wait for rollback before restarting. |
| Staging validation failure category | `Cockpit.InvocationLog.ErrorCategory`; executor JSON error output | `StagingValidationFailed`, with exit code `33` | Not emitted unless the protected staging row count differs from the source count | `FetchRemote` protected staging validation, `Cockpit.Sql.psm1`, and `Invoke-Cockpit.ps1` | The destination replacement is deliberately not applied. The failure can indicate source rows changed during the copy, filtering behavior, triggers, or destination-side row effects. | Read the canonical invocation error for both row counts, confirm the existing destination is intact, investigate source/destination behavior, and retry only after the counts can remain consistent. |
