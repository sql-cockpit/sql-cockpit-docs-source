...
| `POST` | `/api/sql-bridge/invocations/{id}/cancel` | `sqlBridge.invocations.cancel` | Request cancellation through `Cockpit.CancelBridgeInvocation`. The optional JSON `reason` is stored with the request. Hosted tenants route the write through the paired local Agent. |
