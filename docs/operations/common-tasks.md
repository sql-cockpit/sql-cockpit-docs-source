...
- workspace profile sharing:
  storage location: personal profiles are read from `user_preferences.connectionProfiles`, `user_preferences.instanceProfiles`, or `user_preferences.workspace.personal.<workspaceId>`; selected team profiles are written to `settings.workspace.team.<teamId>` as `{ connectionProfiles, instanceProfiles }`
  valid values: any available target workspace other than the active workspace; team targets require active membership; instance profiles are matched by server name, and connection profiles are matched by server/database pair
  default: Connection Manager uses a separate horizontally scrolling drag-and-drop `Share Connections` panel, and Instance Manager uses the matching `Share Instances` panel; workspace names are column headers and destination columns show copied profiles; dragging a connection or instance into another workspace copies that profile plus related profiles; connection `Revoke` removes one copied connection after confirmation; instance `Revoke` removes the copied instance plus matching connections for the same server; instance `Revoke All` removes all instance and connection profiles from that destination workspace after exact workspace-name confirmation; connection `Revoke All` removes all connection profiles from that destination workspace after exact workspace-name confirmation; `Share All Instances` and `Share All Connections` require confirmation before copying the current manager list plus related profiles; existing target profiles are skipped instead of duplicated
