...
- confirmed: the docs host exposes `GET /health` for orchestration liveness probes in both MkDocs live-reload mode and static fallback mode.
...
- discover visible SQL Server instances from `Instance Manager` when you do not already know the host name
...
4. In `Instance Manager`, optionally run `Discover SQL Servers` to find visible instances on the network, then save a low-risk profile and run `Test Connection` to confirm it reaches the expected SQL Server and exposes the expected inventory.
...
- `Agent Jobs` lives under `Operations & Monitoring` and expands to `Agent Manager`, `Runtime Analysis`, and `Job Run History`; `Agent Runtime Comparison` remains a separate operations page.
...
- `Server Explorer`, `Visual Server Explorer`, `Views`, `Stored Procedures`, `SSIS Inspector`, `SQL Editor`, and `Query Analyser` live under `Development & Engineering`. `Views` expands to `View Mapper` and `View Repointer`; `Stored Procedures` expands to `Stored Procedure Mapper` and `Stored Procedure Repointer`. Permission-gated entries remain hidden when the current user lacks access.
...
- connection manager:
  storage location: browser local storage key `sql-cockpit-database-connection-profiles` for saved database connection profiles; live response objects are read on demand through `POST /api/servers/explorer`
  valid values: any saved instance profile plus a database name visible to that profile; saving a database profile also requires a non-empty connection name
  default: the operator selects a saved instance profile first, the server is inherited from that profile without a separate Connection Manager field, `Refresh Databases` loads visible databases through the instance credentials, `Create Profiles For All Databases` creates missing database profiles for the visible database list and skips existing server/database pairs, the server check runs only after the operator clicks `Test Connection`, and saved profiles persist only in the current browser profile on the current machine
  ...
  operational risk: medium, because saved profiles live in browser local storage, the server check exposes live server inventory detail for the selected SQL Server, and bulk creation can expose many database names in the active workspace profile list; low for write safety because the connection-management workflow itself remains read-only; network discovery risk is handled in Instance Manager, not Connection Manager
  safe change procedure: save low-risk connections first, define and test the owning instance in Instance Manager, keep connection names unique enough that operators can tell production and non-production profiles apart, use `Refresh Databases` to confirm the visible database list before bulk creation, use `Edit` to repopulate the draft panel before editing or testing it, delete generated profiles that should not be reused, and run the server check only against low-risk SQL Servers when validating a new connection
- workspace profile sharing:
  storage location: personal profiles are read from `user_preferences.connectionProfiles`, `user_preferences.instanceProfiles`, `user_preferences.remoteInstanceProfiles`, or `user_preferences.workspace.personal.<workspaceId>`; selected team profiles are written to `settings.workspace.team.<teamId>` as `{ connectionProfiles, instanceProfiles, remoteInstanceProfiles }`
  valid values: any available target workspace other than the active workspace; team targets require active membership; instance profiles are matched by server name, connection profiles are matched by server/database pair, and remote source profiles are matched by server/profile label
  default: Connection Manager uses a separate horizontally scrolling drag-and-drop `Share Connections` panel, Instance Manager uses the matching `Share Instances` panel, and Remote Sources uses `Share Remote Sources`; workspace names are column headers and destination columns show copied profiles; dragging a connection or instance into another workspace copies that profile plus related profiles; dragging a remote source copies only that remote source; remote sources default `readOnly` to true and are separate from normal Instance Manager checks; connection `Revoke` removes one copied connection after confirmation; instance `Revoke` removes the copied instance plus matching connections for the same server; remote source `Revoke` removes only the copied remote source; `Revoke All` actions require exact workspace-name confirmation
  code paths affected: `webapp/components/dashboard-client.js`, `webapp/server.js`, and `webapp/lib/rbac-auth-store.js`
  operational risk: medium to high for credential exposure when SQL-auth profiles are shared to a team workspace; medium for target confusion because switching workspace changes which profile IDs and credentials SQL Cockpit resolves
  safe change procedure: confirm the selected destination workspace, share least-privilege profiles first, use `Share All` only after reviewing the confirmation warning, switch to the target workspace, test one shared instance and one shared connection, use instance `Revoke` when both server-level and same-server database-level access should be removed, use connection `Revoke` when only one database connection should be removed, and use `Revoke All` only after typing the exact workspace name
- instance manager:
  ...
  valid values: any non-empty server name that the current auth mode can reach; auth mode per saved instance is `Integrated` or `SQL`; saving a profile also requires a non-empty instance name; SQL-auth drafts require a non-empty `username`; `password` remains optional in the draft, but when present it is written to the paired SQL Cockpit Agent's Windows Credential Manager and the saved profile keeps only password metadata such as `hasPassword` and `secretRef`
  ...
  operational risk: medium for credential handling because SQL-auth passwords depend on the paired agent's Windows Credential Manager scope; medium for metadata exposure because Agent Manager and object-search sync use these profiles to read server-wide metadata
  safe change procedure: validate the instance profile before using Agent Manager, prefer integrated auth only when the `SqlCockpit.Agent` service identity has SQL Server access, re-save SQL-auth passwords after changing the agent service account, and keep database-level profiles in Connection Manager instead of using instance profiles as source or destination database records
- SQL Agent Manager:
  ...
  operational risk: low for write safety because it is read-only; medium for metadata exposure because job names, schedules, messages, and step context can reveal operational details; medium for credential handling when the selected saved profile depends on an agent-side SQL-auth password or an Integrated-auth service identity
  safe change procedure: validate the saved profile in `Instance Manager`, prefer integrated auth only when the agent service identity is the intended SQL principal, refresh a low-risk instance first, verify the source server and job count, then filter before expanding job steps on sensitive instances
- Estate Overview:
  ...
  default: the home page auto-refreshes once when saved instance profiles are available; the browser renders one pending row per saved profile, calls `POST /api/sql-estate/overview` once per instance, and updates rows as responses return; failed instance reads return critical rows instead of failing the entire response
  ...
  operational risk: low for write safety because it is read-only; medium for metadata exposure because capacity, database state, edition, version, and Agent counts can reveal estate details; medium for refresh load because the dashboard can run up to six per-instance requests concurrently
