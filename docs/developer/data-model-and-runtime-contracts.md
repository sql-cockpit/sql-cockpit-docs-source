...
| Credentials | Runtime SQL-auth passwords must be held by the paired SQL Cockpit Agent credential store; hosted/cloud state should persist only metadata such as profile ids, usernames, `hasPassword`, and `secretRef`. |
