...
3. Switch to another workspace and confirm it becomes current.
4. Open `/teams` and confirm it redirects to `/workspaces`.
5. Open `/account` and confirm **Join a team** only shows invite-backed teams.
6. Invite a non-member email from a team row on `/workspaces`, then retry with an active member or duplicate pending email and confirm validation blocks it.
