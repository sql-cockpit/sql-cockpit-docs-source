...
LDAP sign-in binds as the submitted user and then reads standard Active Directory attributes for the local SQL Cockpit profile. New LDAP users are provisioned with `sAMAccountName` as the local username, `mail` as email when present, and a human display name from `displayName`, or `givenName` + `sn` when `displayName` is missing or email-shaped. If `mail` is empty, an email-shaped UPN is used as the email fallback.

Admins can tighten new external account creation from `/admin/signup-allow-list`. The default is `externalSignupAllowListEnabled=false`, which allows all users who can authenticate through enabled external providers to sign up when auto-provisioning is enabled. Enabling the allow list permits only listed LDAP/OIDC subjects, usernames, or emails to create a new local account; existing linked identities can still sign in.

Hosted tenants may set `SQL_COCKPIT_LDAP_VIA_AGENT=true` so LDAP credentials are never sent from the cloud API directly to the customer directory. In that mode, the hosted API leases `identity.ldap.authenticate` work to the paired Agent, then the Azure SQL auth store creates or links the SQL Cockpit user/session from the returned LDAP claims. A hosted LDAP sign-in needs all three parts: LDAP enabled in provider settings, an online paired Agent with `identity.ldap.authenticate`, and Agent-side LDAP connection settings configured from Service Control.
...
| `SQL_COCKPIT_LDAP_URL` | `SQL_COCKPIT_LDAP_URL` env var (optional CLI `--ldapUrl`) | `ldap://...` or `ldaps://...` URL | empty | `enterprise-mode.js` LDAP bind and AD attribute lookup path in `authenticateLdapCredentials` | Login hard-fails in LDAP mode when missing/invalid; profile enrichment also depends on RootDSE/search visibility for the bound user. | Prefer `ldaps://`, test with one service account/user in staging, then roll to production. Confirm the admin user list shows expected username/email/display name. | Confirmed |
...
| `SQL_COCKPIT_LDAP_BIND_USERNAME` | `SQL_COCKPIT_LDAP_BIND_USERNAME` env var (optional CLI `--ldapBindUsername`) | Non-secret LDAP username or UPN used to prefill admin LDAP search bind username | empty | `server.js` admin provider settings response and LDAP search fallback; `/admin/signup-allow-list` bind username field | Low by itself because it is only a username, but using a privileged account for searches increases directory exposure risk. | Use a least-privileged directory search account, keep its password out of environment variables, and verify the admin page pre-fills only the username. | Confirmed |
| `SQL_COCKPIT_LDAP_BIND_USER` | `SQL_COCKPIT_LDAP_BIND_USER` env var (optional CLI `--ldapBindUser`; Node also accepts `LDAP_BIND_USER`) | LDAP service bind DN or UPN | empty | `enterprise-mode.js` service-account LDAP search before user-password validation and admin allow-list lookup/listing | A privileged bind account can expose more directory metadata than intended. Some AD service accounts bind only with their full DN, not UPN. | Use a dedicated least-privileged directory read/search account, not a human admin account. Prefer the exact bind DN when testing service-account search. | Confirmed |
| `SQL_COCKPIT_LDAP_BIND_PASSWORD` | `SQL_COCKPIT_LDAP_BIND_PASSWORD` env var (optional CLI `--ldapBindPassword`; Node also accepts `LDAP_BIND_PASS`) | Secret password for `SQL_COCKPIT_LDAP_BIND_USER` | empty | `enterprise-mode.js` service bind path | Secret leakage risk; incorrect value breaks LDAP login/search when service search is configured. | Store as a secret in production, keep `.env.local` out of commits, restart API, test one valid and one invalid user login. | Confirmed |
| `SQL_COCKPIT_LDAP_SEARCH_BASE` | `SQL_COCKPIT_LDAP_SEARCH_BASE` env var (optional CLI `--ldapSearchBase`; Node also accepts `LDAP_SEARCH_BASE`) | LDAP base DN, for example `DC=example,DC=com` | RootDSE default naming context | `enterprise-mode.js` service-account user search and admin LDAP search | Wrong base returns zero users or wrong-domain users. | Confirm base with AD admins, test exact-user search before enabling allow-list restriction. | Confirmed |
| `SQL_COCKPIT_LDAP_SEARCH_FILTER` | `SQL_COCKPIT_LDAP_SEARCH_FILTER` env var (optional CLI `--ldapSearchFilter`; Node also accepts `LDAP_SEARCH_FILTER`) | LDAP filter template such as `(sAMAccountName={{username}})`; placeholders: `{{username}}`, `{{rawUsername}}`, `{{shortUsername}}`, `{{userPrincipalName}}`, `{{bindUser}}` | empty; SQL Cockpit automatically matches UPN, `sAMAccountName`, and email for exact-user service search | `enterprise-mode.js` exact-user service search for login and non-blank admin LDAP search | Wrong filter can block valid users or match unintended users. | Leave blank first. Add a custom filter only when the directory requires a non-standard lookup, then test UPN and short-name input before production. | Confirmed |
...
| `SQL_COCKPIT_LDAP_VIA_AGENT` | `SQL_COCKPIT_LDAP_VIA_AGENT` env var, Terraform tenant setting `ldap_via_agent`, or persisted LDAP provider setting when env is unset | Boolean: `true` or `false` | `false` unless set by tenant Terraform/provider settings | `server.js` LDAP login route; when true, `/api/auth/login` creates an Agent RPC for `identity.ldap.authenticate` and the hosted auth store creates/links the SQL Cockpit session from verified claims | Login fails closed if no paired Agent is online, the Agent lacks LDAP settings, or hosted auth/RBAC state cannot create the external session. | Enable in test first, configure the lane Agent LDAP plugin in Service Control, confirm dashboard Agent capabilities include `identity.ldap.authenticate`, then test one allowed LDAP user. | Confirmed |
| `SQL_COCKPIT_AGENT_LDAP_TIMEOUT_MS` | `SQL_COCKPIT_AGENT_LDAP_TIMEOUT_MS` env var or Terraform tenant setting `agent_ldap_timeout_ms` | Integer milliseconds, clamped from 1000 to 60000 | `20000` | `server.js` agent-backed LDAP RPC wait timeout | Too low causes false timeouts on slow directory searches; too high keeps login requests open longer. | Start with 20000, inspect Agent and hosted API logs for timeout events, then tune per tenant. | Confirmed |
| `SQL_COCKPIT_AUTH_ALLOW_EMAIL_MATCH_FOR_LINKING` | `SQL_COCKPIT_AUTH_ALLOW_EMAIL_MATCH_FOR_LINKING` env var or Terraform tenant setting `allow_email_match_for_linking` | Boolean: `true` or `false` | `false` | Hosted Azure SQL auth store external-session creation after LDAP/OIDC authentication | If enabled, a verified external provider email can link to an existing local SQL Cockpit account with the same email. This should only be enabled when the external provider email attribute is trusted. | Enable per tenant, validate one known existing local account can sign in through LDAP, then review linked identities and audit logs. | Confirmed |
...
- Hosted LDAP login returns `LDAP sign-in failed` after the Agent authenticates:
  - check hosted API logs for `HOSTED_AUTH_STORE_METHOD_NOT_IMPLEMENTED` or Azure SQL auth-store errors.
  - confirm `SQL_COCKPIT_LDAP_VIA_AGENT=true` and the paired Agent is online with `identity.ldap.authenticate`.
  - confirm auto-provisioning/allow-list policy permits the LDAP subject, username, or email.
  - if logs say the LDAP email matched an existing SQL Cockpit user, enable `SQL_COCKPIT_AUTH_ALLOW_EMAIL_MATCH_FOR_LINKING=true` only when LDAP email is trusted.
- No OTLP audit data:
