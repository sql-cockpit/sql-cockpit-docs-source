...
- Connection Manager shows saved connections in a table with profile, server, database, auth, and action columns. The separate full-width, horizontally scrolling `Share Connections` panel still uses workspace columns where connections can be dragged into another workspace to copy them. Copied connections appear in the destination workspace column.
- Clicking **Edit** for a saved connection opens `/connection-manager/edit?profileId=<id>` and loads that profile into the edit draft. New connections still use `/connection-manager/new`.
- Instance Manager shows saved instances in a table with profile, server, auth, search-sync status, and action columns. The separate full-width, horizontally scrolling `Share Instances` panel still uses workspace columns where instances can be dragged into another workspace to copy them. Copied instances appear in the destination workspace column.
